...
public class EditDonationFrame extends JFrame {
	...
	private EditDonationController controller;
	private JTextField tenJtf;
	private JButton CancelBtn;
	private JButton EditBtn;

	public EditDonationFrame(DonationsController parentController, JFrame parentJFrame) {
		init();
		this.parentController = parentController;
		this.parentFrame = parentJFrame;
		this.parentFrame.setEnabled(false);
		this.phiUngHoBean = new PhiUngHoBean();
		controller = new EditDonationController();
	...
	private void init() {
		setTitle("Sửa thông tin tiền ủng hộ");
	...
	private void EditBtnActionPerformed(java.awt.event.ActionEvent evt) {
		if (validateForm()) {
			...
			try {
				if (this.controller.editDonation(this.phiUngHoBean, parentController.getSelectedIdDonation())) {
